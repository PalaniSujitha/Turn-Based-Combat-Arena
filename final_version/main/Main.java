...
public class Main 
{
    public static void main(String[] args) 
    {
        while( true ) // home page
        {
            ...
            while( true )
            {
                Player player_game = (choice == 1) ? new Warrior("Warrior") : new Wizard("Wizard");
                Level level_game = LevelFactory.createLevel(diff);
                player_game.setInventory(items);

                UI.printGameAttributes( player_game, level_game, items , new SpeedBasedStrategy() );
                BattleManager manager = new BattleManager( player_game, level_game, new SpeedBasedStrategy() );
                manager.startBattle();
                int option = UI.printOption();
                if( option == 1 ) continue;
                else if( option == 2 ) break;
                else if( option == 3 ) exit = true; break;
            }
